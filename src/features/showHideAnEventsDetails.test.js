...
defineFeature(feature, test =>{
    ...
    test('An event element is collapsed by default', ({ given, when, then }) => {
        ...
        then('all events details should be hidden.', () => {
            AppWrapper.update();
            expect(AppWrapper.find('.event p.description')).toHaveLength(0);

        });
    });

    test('User click the details button', ({ given, when, then }) => {
        ...
        then('the event details should be displayed.', () => {
           expect(AppWrapper.find('.event p.description')).toHaveLength(1);
        });
    });
